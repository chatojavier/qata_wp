...
class ACF_Archive {
    ...
    private function add_menu( $label, $menu ) {
        $page_name = sprintf( __( '%s Page', 'acf-archive' ), $label);
    ...
    public function render_menu() {
        ...
        <div class="wrap">
            <h1><?php printf( __( '%s Page', 'acf-archive' ), $post_type_object->label ); ?></h1>
    ...
    public function location_rules_values_archive( $choices ) {
        foreach( $this->get_custom_post_types() as $post_type => $post_type_object ) {
            $choices[ $post_type ] = sprintf( __( '%s Page', 'acf-archive' ), $post_type_object->label );
        }
